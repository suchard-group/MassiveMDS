...
#ifndef BOOST_COMPUTE_ALGORITHM_REDUCE_FAST_HPP
#define BOOST_COMPUTE_ALGORITHM_REDUCE_FAST_HPP
...
#define VPT_DEF 8
#define TBP_DEF 512 //128
#include "reduce_on_gpu_fast.hpp"
...
inline void dispatch_reduce_fast(InputIterator first,
                                 InputIterator last,
                                 const buffer_iterator<T> result_begin,
                                 const buffer_iterator<T> result_end,
                                 const plus<T> &function,
                                 command_queue &queue)
{
    reduce_on_gpu_fast(first, last, result_begin, result_end, function, queue);
}
                                 ...
                                 command_queue &queue)
{
    ...
    array<T, 4096> tmp(context);
    dispatch_reduce_fast(first, last, tmp.begin(), tmp.end(), function, queue);

    // copy result to host
    std::vector<T> host(4096);
    copy(tmp.begin(), tmp.end(), host.begin(), queue);

    *result = std::accumulate(host.begin(), host.end(), static_cast<T>(0));
...
/// \see accumulate()
template<class InputIterator, class OutputIterator, class BinaryFunction>
inline void reduce_fast(InputIterator first,
                        InputIterator last,
                        OutputIterator result,
                        BinaryFunction function,
                        command_queue &queue = system::default_queue())
{
    ...
    detail::dispatch_reduce_fast(first, last, result, function, queue);
}
                        ...
                        command_queue &queue = system::default_queue())
{
    ...
    detail::dispatch_reduce_fast(first, last, result, plus<T>(), queue);
}

} // end compute namespace
} // end boost namespace

#endif // BOOST_COMPUTE_ALGORITHM_REDUCE_FAST_HPP
