...
#ifndef BOOST_COMPUTE_ALGORITHM_DETAIL_REDUCE_ON_GPU_FAST_HPP
#define BOOST_COMPUTE_ALGORITHM_DETAIL_REDUCE_ON_GPU_FAST_HPP
...
inline void reduce_on_gpu_fast(InputIterator first,
                          InputIterator last,
                          buffer_iterator<T> result_begin,
                          buffer_iterator<T> result_end,
                          Function function,
                          command_queue &queue)
{
    ...
    size_t count = std::distance(first, last);
    size_t result_size = std::distance(result_begin, result_end);
    ...
    if (std::ceil(float(count) / vpt / tpb) <= result_size) {

    	reduce_kernel.set_arg(0, first.get_buffer());
    	reduce_kernel.set_arg(1, uint_(0));
    	reduce_kernel.set_arg(2, uint_(count));
    	reduce_kernel.set_arg(3, result_begin.get_buffer());
    	reduce_kernel.set_arg(4, uint_(0));
...
// 		std::cerr << "First = " << std::distance(first, last) << std::endl;
...
// 				std::cerr << "Next = " << count << std::endl;
		...
		reduce_kernel.set_arg(2, uint_(count));
		reduce_kernel.set_arg(3, result_begin.get_buffer());
		reduce_kernel.set_arg(4, uint_(result_begin.get_index()));
...
#endif // BOOST_COMPUTE_ALGORITHM_DETAIL_REDUCE_ON_GPU_FAST_HPP
